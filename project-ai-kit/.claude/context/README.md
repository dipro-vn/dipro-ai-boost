...
|---|---|---|
| `specification.md` | Business context, epics, phase-gate | `ba-agent` |
| `technical.md` | Tech stack, CI/CD, known bugs | `techlead-design-agent`, `backend-agent` |
| `backlog-workflow.md` | Dipro Backlog Rule V2.0 — 6 Issue Types, 9-status flow, Template_Task/Bug (Producer/Bug type/Root Cause), phân quyền, quy tắc subtask | `techlead-tasks-agent` (§I.2 Issue Types + §I.3 Category) · `qc-agent` (§I.6 Status + §III Template_Bug) · `ba-agent` (§I.2 Issue Types) |
| `doc-structure.md` | Cấu trúc SPEC/DESIGN/tasks theo feature type (single-repo vs cross-repo) | `ba-agent`, `techlead-design-agent`, `techlead-tasks-agent`, `designer-agent` |
...
| `business-flows/` (optional) | Business-flow index theo domain — chỉ cần nếu dự án có nhiều domain nghiệp vụ phức tạp, xem pattern trong `business-flows/README.md` | `ba-agent`, `techlead-design-agent` |
