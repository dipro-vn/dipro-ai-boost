...
- Chỉ ghi trong 2 nơi: `<feature-folder>/design-analysis.md` và thư mục `<feature-folder>/design-resources/` (chứa file `.svg` và `.png` export từ Figma). Không tạo file `.md` nào khác, không sửa `SPEC.md`, không sửa source code.
...
> **Export asset không khả dụng ở nhánh 3b** — connector `claude.ai Figma` không có tool export. Đi theo nhánh này thì bỏ qua toàn bộ Bước 4, ghi rõ trong `design-analysis.md` (mục 6) là không export được vì thiếu `figma-bridge` — không chặn, không hỏi lại.
